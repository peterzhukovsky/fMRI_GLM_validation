...
load(char(strcat('G:\Impulsivity_project\DCM\MID\', files(i),'\Deconv_rIFG_ts_deconv')))
rIFG=data_deconv; rIFG=rIFG(1:660); rIFG_time=(2:2:1320); xq = 1:0.01:1320;
rIFG_interp = interp1(rIFG_time,rIFG,xq,'spline');

load(char(strcat('G:\Impulsivity_project\DCM\MID\', files(i),'\Deconv_ACC_ts_deconv')))
acc=data_deconv; acc=acc(1:660); acc_time=(2:2:1320); xq = 1:0.01:1320;
acc_interp = interp1(acc_time,acc,xq,'spline');

load(char(strcat('G:\Impulsivity_project\DCM\MID\', files(i),'\Deconv_CAU_ts_deconv')))
...
%concatenate across subjects
    ...
    subject_id_all=vertcat(subject_id_all, subject_id');
        rIFG_plot_all=vertcat(rIFG_plot_all, rIFG_plot);
                                    ...
                                    pred_features=horzcat(rIFG_activation', acc_activation', CAU_activation');
                                    ...
                                    AUC_all(i)=AUClog; betas(i)=model1.Coefficients{4,1};
...
line([1 400], [0 0],'color','r','linewidth',3); line([400 700], [0 0],'color','k','linewidth',3); %x-axis 
line([100 100], [-.2 .25],'color','k','linewidth',1); %ylim([-.1, 0.2]);
line([500 500], [-.2 .25],'color','k','linewidth',1); 
%https://uk.mathworks.com/matlabcentral/answers/40969-drawing-x-and-y-axis-on-2d-plot
%https://uk.mathworks.com/matlabcentral/answers/180829-shade-area-between-graphs


m_prem=mean(rIFG_plot_prem); m_cor=mean(rIFG_plot_cor);
plt=area([100:410], m_prem(100:410), 'LineWidth',0.5);plt.FaceColor = [1 0 0]; alpha(plt,.1); 
plt=area([100:500], m_prem(100:500), 'LineWidth',0.5);plt.FaceColor = [1 0 0]; alpha(plt,.3); 
plt=area([100:590], m_prem(100:590),'LineWidth',0.5);plt.FaceColor = [1 0 0]; alpha(plt,.5); 
xlabel('time in s/10'); ylabel('ROI activation');ylim([-.1, 0.2]);
...
pred_features=horzcat(IFG_all,acc_all,CAU_all)
